...
// Зададим фиктивный массив «городов» для рандома (имена и фиас)
const mockCities = [
  { city_name: "Домодедово", city_fias_id: "some-fias-123" },
  { city_name: "Подольск", city_fias_id: "some-fias-456" },
  { city_name: "Химки", city_fias_id: "some-fias-789" },
  { city_name: "Мытищи", city_fias_id: "some-fias-abc" },
  { city_name: "Люберцы", city_fias_id: "some-fias-xyz" },
];

const useNewIncidentStore = create((set, get) => ({
  // Заполняем форму случайными данными (включая фиктивный город)
  fillFormWithMagic: (form) => {
    ...
    // Подставляем в randomValues
    randomValues.addressInfo = {
      ...randomValues.addressInfo,
      city_name: cityObj.city_name,
      city_fias_id: cityObj.city_fias_id,
    };

    // Приводим даты к dayjs
  ...
  // Отправка 10 инцидентов
  sendMultipleIncidents: async (token) => {
    const promises = [];

    for (let i = 0; i < 10; i++) {
      const values = getRandomNewIncidentFields();
      ...
      const start_time = dayjs(values.start_time).format("HH:mm") + ":00.000";

      const est_date = dayjs(values.estimated_restoration_date).format(
        "YYYY-MM-DD"
      );
      const est_time =
        dayjs(values.estimated_restoration_time).format("HH:mm") + ":00.000";
      ...
      // Формируем описание для Strapi (rich text)
      const description = [
        {
          type: "paragraph",
