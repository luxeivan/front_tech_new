...
// утилиты/хуки/компоненты
import { val, formatDate } from "@/lib/format";
import FiltersBar from "./components/FiltersBar";
import TnTable from "./components/TnTable";
import useTnFilters from "./hooks/useTnFilters";
...
export default function MainPage() {
  ...
  // SSE подписка
  useSse(status === "authenticated", handleEvent);
  ...
  // сортировка и плоские данные для таблицы
  ...
  // фильтры
  const { filters, setFilters, filterOptions, applyFilters } =
    useTnFilters(uniqueOpen);

  const filteredData = useMemo(
    () => dataSource.filter((row) => applyFilters(row.full, filters)),
    [dataSource, applyFilters, filters]
  );

  // редактирование полей (модалка)
